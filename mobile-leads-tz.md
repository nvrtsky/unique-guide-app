---
title: "ТЗ MVP: мобильные лиды, туристы и сводная тура"
subtitle: "Финальный ориентир для кликабельного прототипа и последующего переноса в рабочее приложение. Состав полей и бизнес-логика берутся из веб-версии, компоновка адаптируется под телефон."
date: "4 августа 2026"
lang: ru
---
...
| Прототип | `mobile-leads.html`, `tour-operations.html` и связанные mock-файлы |
...
<div class="final-map" role="img" aria-label="Рабочая мобильная основа и web-контракт складываются в одну финальную мобильную CRM">
  <article><strong>Текущая GuideMobile</strong><span>Авторизация, роли, API-клиент, кэш, loading/error, safe-area и действующее нижнее меню.</span></article>
  <article><strong>Этот MVP-прототип</strong><span>Полные Lead, Tourist и Tour surfaces, вертикальная сводная, группы, конфликты и карточные формы.</span></article>
  <article><strong>Финальная версия</strong><span>Одна рабочая GuideMobile на production-данных. Отдельного второго приложения и дублирующих карточек не будет.</span></article>
</div>
...
1. `GuideMobile.tsx` останется оболочкой авторизации и навигации, а Lead/Tour/Tourist/Summary будут вынесены в отдельные mobile-компоненты.
...
7. Mock/localStorage исчезнут. Все записи пройдут через текущие API или отдельный mobile facade с транзакциями, идемпотентностью и server capabilities.
...
Для admin и manager «Лиды» и «Туры» остаются разными пунктами меню:
...
| `viewer`, подпись «Гид» | Read-only профиль и статусы только назначенных городов |
...
- **SCOPE-02.** Нижняя навигация admin и manager содержит только «Туры», «Туристы» и «Лиды». Роли `viewer` и `escort` видят только «Туры» и «Туристы». Отдельный раздел с оплатами в задачу не входит.
- **SCOPE-03.** Для admin и manager «Лиды» и «Туры» остаются разными разделами. Из назначенного и доступного лида можно открыть сводную тура с фильтром по этому лиду.
- **SCOPE-04.** Прежнее ТЗ приложения гида и рабочие приложения не изменяются; меняется только изолированный MVP-прототип текущей задачи.
...
## 1.2. Передача MVP без слияния

Текущая разработка хранится только в ветке `prototype/mobile-crm-2026-08-03`. Этот этап не создаёт PR, не сливается в `main` и не меняет источник GitHub Pages. Публичная страница из `main` не считается preview этой ветки.

Финальная передача содержит имя ветки, точный commit SHA и preview или архив, собранный из этого SHA. Ссылка только на имя ветки не фиксирует версию, потому что ветка может получить новые коммиты. Контрольная сумма Markdown, сгенерированный HTML, скриншоты и результаты smoke относятся к одному commit SHA.

- **DELIVERY-01.** Все файлы MVP находятся в `prototype/mobile-crm-2026-08-03`; `main` и `develop` не меняются.
- **DELIVERY-02.** Для текущего этапа не создаются PR, merge и GitHub Pages deployment.
- **DELIVERY-03.** Передача указывает точный commit SHA и даёт неизменяемый preview или архив этого коммита.
- **DELIVERY-04.** HTML, скриншоты и результаты проверок воспроизводятся из указанного SHA без незакоммиченных файлов.
...
Для admin и manager «Лиды» и «Туры» остаются самостоятельными пунктами, как в веб-версии. «Туристы» даёт быстрый вход в ту же единую карточку туриста, которая открывается из лида и сводной. Роли `viewer` и `escort` не получают пункт «Лиды», список лидов или карточку лида. Отдельной нижней вкладки «Финансы» нет.

- **IA-01.** Для admin и manager нижнее меню идёт строго `Туры → Туристы → Лиды`; для viewer и escort оно идёт `Туры → Туристы` без пункта «Лиды».
...
- **TOUR-03.** Форма изменения сохраняет все поля `EditEventDialog` от названия до цвета и не добавляет вкладки «Финансы» или «Расходы».
...
Роли `viewer` и `escort` не получают отдельные поля фамилии, имени и отчества, email, паспорт РФ, кем выдан, адрес регистрации, `notes`, `contactId`, `leadTouristId`, `leadId`, `dealId`, признак представителя группы, признак основного туриста заявки, поля оплаты, цену или сайт тура. Кнопка и deep link исходного лида им недоступны. Действие «Написать» использует нейтральную подпись «Чат» и не раскрывает внутреннее поле предпочтительного канала.
...
- **AC-01.** Нижняя навигация admin и manager содержит «Туры», «Туристы» и «Лиды» в этом порядке; роли `viewer` и `escort` видят только «Туры» и «Туристы». Отдельной вкладки «Финансы» нет.
...
- **AC-56.** Финальная передача указывает ветку `prototype/mobile-crm-2026-08-03`, точный commit SHA и неизменяемый preview или архив этого SHA; PR, merge в `main` и GitHub Pages deployment отсутствуют.
...
- **NG-06.** Не добавлять отдельные вкладки «Финансы» и «Расходы». Существующие web-поля оплаты лида и базовой цены тура сохраняются для паритета.
...
- **NG-14.** Не создавать PR, не сливать MVP в `main` или `develop` и не публиковать прототип через GitHub Pages на текущем этапе.
...
14. Переключить admin, manager, escort и viewer; проверить закрытый перечень персональных полей, отсутствие «Лидов» у viewer/escort, только назначенные лиды manager и запрещённые действия. Отдельно открыть legacy `role=guide` и убедиться, что он нормализован в viewer, а неизвестная роль не получает manager.
...
21. Передать ветку `prototype/mobile-crm-2026-08-03`, точный commit SHA и preview или архив этого SHA. Проверить отсутствие PR, merge в `main` и GitHub Pages deployment.
