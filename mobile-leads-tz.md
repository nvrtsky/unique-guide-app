...
| Менеджер | Профиль, документы, логистика и группы назначенных лидов |
...
1. Основная сводная: открыть `tour-operations.html?tourId=china&role=manager&view=operations`.
2. Конфликт: нажать «Объединить», выбрать туристов с разными данными, перейти дальше и выбрать запись-источник.
3. Разъединение: открыть состав общей операции и нажать «Отделить».
4. Роль гида: открыть `tour-operations.html?tourId=china&role=guide&view=tourists&tourist=t1`.
5. Рабочие статусы: открыть `tour-operations.html?tourId=china&role=manager&view=work` и нажать карточку участника.
6. Offline: открыть `tour-operations.html?tourId=china&role=manager&view=operations&offline=1`.
7. Справочник и состояния: открыть меню тура; выбрать «Города и точки» или «Состояния экрана» в меню роли.
