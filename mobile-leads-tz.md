---
title: "ТЗ: единая мобильная CRM для менеджеров и гидов"
...
| Главная страница прототипа | [Открыть мобильную CRM](tour-operations.html) |
| Стоимость всей текущей разработки | [390 000 ₽ за оба шага, подробный расчёт](commercial-proposal.html) |
| Срок разработки | **30 календарных дней** после письменного утверждения прототипа и получения необходимых доступов |
...
<div class="prototype-note"><p><strong>Это финальный ориентир для согласования.</strong> Сначала клиент проверяет понятные сценарии и экраны ниже. Точные поля, права и технические проверки находятся в свёрнутом приложении в конце документа.</p></div>

## Как читать это ТЗ

Документ состоит из двух частей:

1. **Клиентская часть** показывает итоговый продукт, основные действия и экраны.
2. **Техническое приложение** фиксирует все поля, права, проверки и ограничения для команды разработки.

Текущий этап посвящён согласованию кликабельного прототипа. Он показывает, как менеджер будет работать с лидами, туристами, турами, рейсами, отелями, отъездами, чатами и оплатой с телефона. Данные тестовые; рабочая база на этом этапе не меняется.
...
<div class="final-map" role="img" aria-label="Рабочая мобильная основа и web-контракт складываются в одну финальную мобильную CRM">
  <article><strong>Уже работает для гида</strong><span>Задачи по маршруту, туристы, доступные финансы, статусы и работа без связи остаются без изменений.</span></article>
  <article><strong>Добавляется для менеджера</strong><span>Лиды, полные карточки, мобильная сводная, группы туристов, логистика, документы, чаты и оплаты.</span></article>
  <article><strong>После объединения</strong><span>Одно мобильное приложение с общей навигацией. Каждый пользователь видит только разрешённые ему разделы и действия.</span></article>
</div>

<div class="scope-bands" aria-label="Что входит в текущий и следующий этапы">
  <article class="scope-band prototype"><strong>Шаг 1: кликабельный прототип</strong><span>Сейчас можно открыть и проверить</span><ul><li>единое меню и роли;</li><li>карточки лида, туриста и тура;</li><li>сводную, группы и конфликты;</li><li>чаты, финансы и справочник;</li><li>тестовые состояния без записи в рабочую базу.</li></ul></article>
  <article class="scope-band next"><strong>Шаг 2: рабочая версия</strong><span>Входит в общую стоимость 390 000 ₽</span><ul><li>подключение рабочих данных;</li><li>серверные права;</li><li>массовые безопасные операции;</li><li>тестирование и выпуск.</li></ul></article>
  <article class="scope-band backlog"><strong>Не меняется</strong><span>Сохраняем как есть</span><ul><li>веб-версия CRM;</li><li>первая редакция для гида;</li><li>существующие лиды, туры и туристы;</li><li>действующая финансовая часть системы.</li></ul></article>
</div>
...
### Срок разработки: 30 календарных дней

Отсчёт начинается, когда клиент письменно утвердил прототип и передал доступы, необходимые для подключения и проверки рабочей версии. Если для продолжения не хватает решения или доступа со стороны клиента, срок сдвигается на время ожидания.

<div class="schedule-map" role="img" aria-label="График разработки на 30 календарных дней">
  <article><b>Дни 1–5</b><strong>Уточняем интерфейс и контракты</strong><span>Фиксируем принятый прототип, права и точки подключения к рабочим данным.</span></article>
  <article><b>Дни 6–20</b><strong>Разрабатываем и интегрируем</strong><span>Переносим согласованные экраны, подключаем серверные проверки, чаты и Wazzup.</span></article>
  <article><b>Дни 21–26</b><strong>Тестируем и исправляем</strong><span>Проверяем роли, данные, мобильные размеры и сценарии первой редакции гида.</span></article>
  <article><b>Дни 27–30</b><strong>Выпускаем и передаём</strong><span>Проводим итоговую приёмку, публикуем рабочую версию и передаём документацию.</span></article>
</div>

### Что именно изменится в рабочем приложении после согласования

1. Гид по-прежнему начинает с «Задач» и видит только назначенные ему города и туристов.
2. У менеджера и администратора появится единое меню: «Туры», «Туристы», «Финансы», «Лиды», «Чаты».
3. Карточка туриста станет одной: вкладка «Профиль» хранит постоянные данные, вкладка «В туре» показывает участие в выбранном туре.
4. В карточке тура появятся обзор, сводная, программа, команда, задачи и действия.
5. Из лида можно будет открыть сводную связанного тура и вернуться назад без потери выбранного места.
6. Общая запись рейса, отеля или отъезда не будет стирать личные данные туристов.
7. «Финансы» сохранят действующую логику остатков, плательщика и отметки оплаты. Новая бухгалтерия не добавляется.
8. «Чаты» соберут служебные и клиентские переписки для менеджера; Wazzup останется внешним каналом конкретного лида.

### Карта единого продукта

<div class="path-map" role="img" aria-label="Путь от лида к сводной тура">
  <div class="path-node"><strong>Лид</strong><span>Заявка, контакт, сообщения, документы и задачи</span></div>
  <div class="path-arrow">→</div>
  <div class="path-node blue"><strong>Туристы</strong><span>Участники заявки с личными данными и документами</span></div>
  <div class="path-arrow">→</div>
  <div class="path-node dark"><strong>Тур и сводная</strong><span>Маршрут, логистика, статусы и оплата всего тура</span></div>
</div>

<div class="operation-triad" aria-label="Три независимые операции в каждом городе"><div class="operation-chip arrival">Прибытие</div><div class="operation-chip hotel">Отель</div><div class="operation-chip departure">Отъезд</div></div>
...
<div class="steps" aria-label="Пять шагов ежедневной работы менеджера">
  <div class="step"><b>1</b><strong>Открыть лид</strong><span>Проверить заявку и состав туристов</span></div>
  <div class="step"><b>2</b><strong>Перейти в тур</strong><span>Нажать «Открыть сводную тура»</span></div>
  <div class="step"><b>3</b><strong>Выбрать город</strong><span>Открыть нужную остановку маршрута</span></div>
  <div class="step"><b>4</b><strong>Заполнить логистику</strong><span>Прибытие, отель и отъезд работают отдельно</span></div>
  <div class="step"><b>5</b><strong>Проверить пробелы</strong><span>Режим «Покрытие» показывает, чего не хватает</span></div>
</div>
...
<div class="grouping-diagram" role="img" aria-label="Одна группа туристов с общим отелем, двумя прибытиями и личными отъездами">
  <div class="people-row"><div class="person-node"><i>АС</i><strong>Анна</strong><span>лид 1</span></div><div class="person-node"><i>ИС</i><strong>Илья</strong><span>лид 1</span></div><div class="person-node"><i>МО</i><strong>Марина</strong><span>лид 2</span></div><div class="person-node"><i>ДВ</i><strong>Денис</strong><span>лид 2</span></div></div>
  <div class="tour-group-bar">Одна группа туристов на весь тур</div>
  <div class="operation-lane"><span class="lane-label">Отель</span><span class="lane-group hotel">Один отель на четверых</span></div>
  <div class="operation-lane"><span class="lane-label">Прибытие</span><span class="lane-group arrival-a">Рейс A · 2</span><span class="lane-group arrival-b">Рейс B · 2</span></div>
  <div class="operation-lane"><span class="lane-label">Отъезд</span><span class="lane-group departure">Личный</span><span class="lane-group departure">Личный</span><span class="lane-group departure">Личный</span><span class="lane-group departure">Личный</span></div>
  <p class="diagram-note">Объединение одной операции не объединяет остальные. Можно жить вместе, прилететь разными рейсами и уехать отдельно.</p>
</div>
...
<div class="value-flow" role="img" aria-label="Личные данные туриста сохраняются под общей записью"><div class="value-node"><strong>Личная запись</strong><span>Данные конкретного туриста не удаляются</span></div><div class="value-arrow">+</div><div class="value-node shared"><strong>Общая запись</strong><span>Показывает выбранный рейс, отель или отъезд группе</span></div><div class="value-arrow">→</div><div class="value-node visible"><strong>Что видно в сводной</strong><span>Общее значение до разъединения, затем личное</span></div></div>
...
<div class="role-grid" aria-label="Четыре роли мобильной CRM"><div class="role-card admin"><b>Администратор</b><span>Все действия, удаление, управление турами и справочником</span></div><div class="role-card manager"><b>Менеджер</b><span>Назначенные лиды, все операции доступного тура и группы туристов</span></div><div class="role-card escort"><b>Сопровождающий</b><span>Разрешённые поля и фактические статусы назначенного тура</span></div><div class="role-card guide"><b>Гид</b><span>Задачи и статусы только назначенных городов, без раздела «Лиды»</span></div></div>
...
Пользователь перед отправкой сообщения должен понимать, кто его получит. Поэтому мобильная CRM не складывает все переписки в одну ленту. У каждого контура свой адресат, история и правила доступа.

<div class="chat-map" aria-label="Три независимых контура чатов">
  <article class="staff"><b>Только сотрудники</b><strong>Команда</strong><span>Общий чат, личные диалоги, группы и комнаты конкретного лида или тура.</span><small>Клиенты эти сообщения не видят.</small></article>
  <article class="clients"><b>Сотрудник и клиент</b><strong>Клиенты внутри UNIQUE</strong><span>Личный чат с туристом и общий чат участников конкретного тура.</span><small>Перед отправкой показан получатель или состав тура.</small></article>
  <article class="wazzup"><b>Внешний канал</b><strong>Wazzup</strong><span>WhatsApp, Telegram, Instagram или Max открывается из карточки лида.</span><small>История и статусы сообщения остаются внутри Wazzup.</small></article>
</div>

#### Что уже работает и что добавляется

В рабочей мобильной версии гид уже может открыть личный чат туриста и общий чат тура. Эти входы и права сохраняются. Сейчас в мобильном приложении нет общего центра чатов сотрудников и нет Wazzup. Прототип показывает, как эти два элемента будут выглядеть после переноса.

У менеджера и администратора появляется пятый пункт нижнего меню «Чаты». В нём три вкладки: «Команда», «Клиенты» и «Wazzup». У гида и сопровождающего нижнее меню остаётся прежним. Они входят в разрешённый чат из шапки тура или карточки туриста.

Wazzup не смешивается с внутренними переписками UNIQUE. Мобильное приложение открывает внешний чат из лида, но не копирует его историю, отметки доставки или прочтения. На экране различаются четыре состояния: ключ сохранён, канал активен, чат загружается, произошла ошибка.

<div class="chat-scenarios">
  <article><b>01</b><strong>Обсудить тур с коллегами</strong><ol><li>Менеджер открывает «Чаты → Команда».</li><li>Выбирает комнату тура или пишет сотруднику лично.</li><li>Отправляет текст либо файл до 10 МБ.</li></ol><p>Сообщение получают только участники служебной комнаты.</p></article>
  <article><b>02</b><strong>Ответить конкретному туристу</strong><ol><li>Менеджер открывает «Чаты → Клиенты».</li><li>Выбирает личный диалог по контакту.</li><li>Проверяет имя получателя и отправляет текст, изображение или PDF.</li></ol><p>Диалог остаётся одним для контакта; выбранный тур задаёт контекст, но не создаёт копию переписки.</p></article>
  <article><b>03</b><strong>Написать всем участникам тура</strong><ol><li>Гид или менеджер открывает общий чат из карточки тура.</li><li>Проверяет название тура и состав участников.</li><li>Отправляет сообщение или вложение.</li></ol><p>Клиенты этого тура видят сообщение в своём общем чате.</p></article>
  <article><b>04</b><strong>Продолжить переписку через Wazzup</strong><ol><li>Менеджер открывает нужный лид.</li><li>Переходит во вкладку «Чат».</li><li>Проверяет мессенджер и получателя, затем работает во внешнем окне Wazzup.</li></ol><p>При ошибке загрузки интерфейс сохраняет контекст лида и предлагает повторить.</p></article>
</div>

<div class="chat-shots">
  <figure><div class="shot-frame"><a href="tour-operations.html?view=chats&amp;chatScope=staff&amp;role=manager"><img src="assets/spec/26-chat-hub-final.png" alt="Центр чатов менеджера с вкладками Команда, Клиенты и Wazzup"></a></div><figcaption><strong>Центр чатов.</strong> Непрочитанные сообщения собраны по типу адресата, а не в общей ленте.</figcaption></figure>
  <figure><div class="shot-frame"><a href="tour-operations.html?view=chats&amp;chatScope=staff&amp;chat=staff-tour-china&amp;role=manager"><img src="assets/spec/27-chat-staff-final.png" alt="Служебная комната тура только для сотрудников"></a></div><figcaption><strong>Команда.</strong> Под заголовком указано, что сообщения видят только сотрудники.</figcaption></figure>
  <figure><div class="shot-frame"><a href="tour-operations.html?view=chats&amp;chatScope=client&amp;chat=client-tourist-t1&amp;role=manager"><img src="assets/spec/28-chat-client-final.png" alt="Личный внутренний чат менеджера с туристом"></a></div><figcaption><strong>Личный чат.</strong> Имя получателя видно до отправки сообщения.</figcaption></figure>
  <figure><div class="shot-frame"><a href="tour-operations.html?view=leads&amp;lead=lead-1042&amp;tab=chat&amp;tourId=china&amp;role=manager"><img src="assets/spec/29-chat-wazzup-final.png" alt="Внешний чат Wazzup в карточке лида"></a></div><figcaption><strong>Wazzup.</strong> Интерфейс показывает мессенджер, получателя и состояние внешнего канала.</figcaption></figure>
  <figure><div class="shot-frame"><a href="tour-operations.html?view=leads&amp;lead=lead-1042&amp;tab=chat&amp;tourId=china&amp;role=manager&amp;wazzup=error"><img src="assets/spec/30-chat-wazzup-error-final.png" alt="Ошибка загрузки внешнего чата Wazzup с повтором"></a></div><figcaption><strong>Ошибка Wazzup.</strong> Лид остаётся открыт, пользователь может повторить загрузку.</figcaption></figure>
  <figure><div class="shot-frame"><a href="tour-operations.html?view=chats&amp;chatScope=client&amp;chat=client-tour-china&amp;tourId=china&amp;role=viewer"><img src="assets/spec/31-chat-guide-final.png" alt="Общий чат тура в прежнем интерфейсе гида"></a></div><figcaption><strong>Гид.</strong> Общий чат открывается из тура; нижнее меню первой редакции не меняется.</figcaption></figure>
</div>
...
<div class="state-gallery" aria-label="Роли и системные состояния">
  <figure class="mini-screen"><div class="shot-frame"><a href="tour-operations.html?tourId=china&amp;role=viewer"><img src="assets/spec/25-guide-final.png" alt="Рабочий экран гида"></a></div><figcaption><strong>Гид.</strong> Задачи и назначенные города остаются такими же, как в первой редакции.</figcaption></figure>
  <figure class="mini-screen"><div class="shot-frame"><a href="tour-operations.html?tourId=china&amp;view=statuses&amp;role=manager"><img src="assets/spec/22-statuses-final.png" alt="Статусы операций по выбранному городу"></a></div><figcaption><strong>Статусы.</strong> Встреча, заселение и отъезд меняются отдельно.</figcaption></figure>
  <figure class="mini-screen"><div class="shot-frame"><a href="tour-operations.html?tourId=china&amp;tourSection=summary&amp;role=manager&amp;offline=1"><img src="assets/spec/11-offline.png" alt="Сводная в режиме без подключения"></a></div><figcaption><strong>Без подключения.</strong> Данные можно читать, но сохранение заблокировано.</figcaption></figure>
</div>

### Границы текущей разработки
...
<div class="acceptance-route" aria-label="Главный сценарий приёмки"><span>4 туриста</span><i>→</i><span>2 лида</span><i>→</i><span>1 группа тура</span><i>→</i><span>общий отель на 4</span><i>→</i><span>прибытия 2 + 2</span><i>→</i><span>личные отъезды</span><i>→</i><span>отделить одного</span></div>
...
<div class="state-gallery acceptance-gallery" aria-label="Ошибки, пустые данные и малый экран">
  <figure class="mini-screen"><div class="shot-frame"><img src="assets/spec/13-error-state.png" alt="Сообщение об ошибке загрузки и кнопка Повторить"></div><figcaption><strong>Ошибка.</strong> Понятное объяснение и кнопка «Повторить».</figcaption></figure>
  <figure class="mini-screen"><div class="shot-frame"><img src="assets/spec/14-empty-state.png" alt="Пустой тур с объяснением следующего шага"></div><figcaption><strong>Пустой экран.</strong> Пользователь понимает, почему данных нет и что делать дальше.</figcaption></figure>
  <figure class="mini-screen"><div class="shot-frame"><img src="assets/spec/15-summary-375.png" alt="Мобильная сводная на ширине 375 пикселей"></div><figcaption><strong>375 px.</strong> Интерфейс не требует горизонтальной прокрутки.</figcaption></figure>
</div>

<span id="client-scenarios"></span>
...
<div class="client-scenarios">
  <article>
    <header><b>01</b><div><strong>Из лида в связанный тур</strong><span>Роль: менеджер</span></div></header>
    <ol><li>Открыть карточку Анны Соколовой.</li><li>Проверить состав туристов.</li><li>Нажать «Открыть сводную тура».</li><li>Вернуться в «Лиды».</li></ol>
    <p><strong>Правильный результат</strong> Открывается именно тур этого лида, а после возврата сохраняются выбранный тур и место работы.</p>
    <a href="tour-operations.html?view=leads&amp;lead=lead-1042&amp;tab=details&amp;tourId=china&amp;role=manager">Начать сценарий</a>
  </article>
  <article>
    <header><b>02</b><div><strong>Заполнить логистику тура</strong><span>Роль: менеджер</span></div></header>
    <ol><li>Выбрать остановку маршрута.</li><li>По очереди открыть «Прибытие», «Отель» и «Отъезд».</li><li>Переключить «Этот лид / Весь тур».</li><li>Открыть режим «Покрытие».</li></ol>
    <p><strong>Правильный результат</strong> Менеджер видит, у кого данные уже заполнены, а у кого ещё нет.</p>
    <a href="tour-operations.html?tourId=china&amp;role=manager&amp;tourSection=summary">Начать сценарий</a>
  </article>
  <article>
    <header><b>03</b><div><strong>Объединить туристов</strong><span>4 туриста из 2 лидов</span></div></header>
    <ol><li>Выбрать четырёх туристов.</li><li>Создать одну группу тура.</li><li>Назначить им один общий отель.</li><li>Разделить прибытия на две пары, а отъезды оставить личными.</li></ol>
    <p><strong>Правильный результат</strong> Туристы живут вместе, но могут прилететь разными рейсами и уехать отдельно.</p>
    <a href="tour-operations.html?tourId=china&amp;role=manager&amp;tourSection=summary">Начать сценарий</a>
  </article>
  <article>
    <header><b>04</b><div><strong>Разрешить конфликт и отделить туриста</strong><span>Без потери личных данных</span></div></header>
    <ol><li>Выбрать туристов с разными рейсами.</li><li>Сравнить записи на экране сверки.</li><li>Выбрать основную запись.</li><li>Отделить одного туриста от общей операции.</li></ol>
    <p><strong>Правильный результат</strong> До подтверждения ничего не меняется. После отделения снова видна личная запись туриста, а отель и другие операции остаются прежними.</p>
    <a href="tour-operations.html?tourId=china&amp;role=manager&amp;tourSection=summary">Начать сценарий</a>
  </article>
  <article>
    <header><b>05</b><div><strong>Проверить оплату</strong><span>Роль: менеджер</span></div></header>
    <ol><li>Открыть «Финансы».</li><li>Найти заявку с остатком.</li><li>Проверить плательщика и валюту.</li><li>Отметить получение оплаты и отменить отметку.</li></ol>
    <p><strong>Правильный результат</strong> Одна заявка показывается одной карточкой, суммы разных валют не складываются между собой.</p>
    <a href="tour-operations.html?tourId=china&amp;view=finance&amp;role=manager">Начать сценарий</a>
  </article>
  <article>
    <header><b>06</b><div><strong>Убедиться, что версия гида сохранена</strong><span>Роль: гид</span></div></header>
    <ol><li>Переключиться на роль «Гид».</li><li>Открыть задачи, туристов и статусы своего города.</li><li>Проверить доступные финансы.</li><li>Включить режим без подключения.</li></ol>
    <p><strong>Правильный результат</strong> Гид продолжает работать по знакомому сценарию и не видит лиды или закрытые данные менеджера.</p>
    <a href="tour-operations.html?tourId=china&amp;role=viewer">Начать сценарий</a>
  </article>
</div>
...
<div class="screen-board">
  <figure class="annotated-shot">
    <div class="shot-frame">
      <a href="tour-operations.html?view=leads&amp;lead=lead-1042&amp;tab=details&amp;tourId=china&amp;role=manager"><img src="assets/spec/16-lead-final.png" alt="Карточка лида: вкладки и блоки формы в порядке веб-версии"></a>
      <span aria-hidden="true" class="marker blue" style="--x:19%;--y:19%">1</span>
      <span aria-hidden="true" class="marker" style="--x:90%;--y:34%">2</span>
      <span aria-hidden="true" class="marker purple" style="--x:86%;--y:54%">3</span>
      <span aria-hidden="true" class="marker amber" style="--x:19%;--y:72%">4</span>
    </div>
    <figcaption><strong>Карточка лида.</strong> 1 — те же четыре раздела, что в веб-версии; 2 — проверка связи с туром; 3 — объединение дублей; 4 — поля идут от личных данных к туру и оплате.</figcaption>
  </figure>

  <figure class="annotated-shot">
    <div class="shot-frame">
      <a href="tour-operations.html?tourId=china&amp;tourSection=overview&amp;role=manager"><img src="assets/spec/17-tour-final.png" alt="Обзор тура с шестью разделами и полями веб-карточки"></a>
    ...
    <figcaption><strong>Обзор тура.</strong> 1 — шесть частей карточки; 2 — состояние тура; 3 — маршрут, даты, цена и участники в том же порядке, что в веб-версии; 4 — общее нижнее меню.</figcaption>
  </figure>
  ...
  <figure class="annotated-shot">
    <div class="shot-frame">
      <a href="tour-operations.html?tourId=china&amp;tourist=t1&amp;touristSection=profile&amp;role=manager"><img src="assets/spec/18-tourist-profile-final.png" alt="Базовый профиль туриста с пятью разделами веб-карточки"></a>
    ...
    <figcaption><strong>Профиль туриста.</strong> 1 — постоянные данные или участие в выбранном туре; 2 — связь с лидом и роль туриста в заявке; 3 — готовность и быстрые контакты; 4 — пять разделов в порядке веб-карточки.</figcaption>
  </figure>
  ...
  <figure class="annotated-shot">
    ...
    <figcaption><strong>Команда тура.</strong> 1 — гиды по каждой остановке маршрута; 2 — город сбора оплаты; 3 — сопровождающий; 4 — администраторы чата.</figcaption>
  </figure>
  ...
  <figure class="annotated-shot">
    ...
    <figcaption><strong>Статусы на маршруте.</strong> 1 — отдельный режим сводной; 2 — выбранная остановка; 3 — прибытие, заселение и отъезд по участнику; 4 — понятный выбор нового статуса.</figcaption>
  </figure>
  ...
  <figure class="annotated-shot">
    ...
    <figcaption><strong>Финансы.</strong> 1 — раздел виден только при наличии прав; 2 — итоги показаны отдельно по каждой валюте; 3 — одна карточка на заявку; 4 — плательщик, состав и безопасная отметка «Оплачен».</figcaption>
  </figure>
...
<div class="scenario-flow" aria-label="Главный сценарий группировки">
  <ol>
    ...
    <li><strong>Подтверждение</strong><span>Открывает логистику</span></li>
    <li><strong>Сводная</strong><span>Остановка и операция</span></li>
    <li><strong>Группа тура</strong><span>Туристы связаны на весь тур</span></li>
    <li><strong>Общая операция</strong><span>Отель, прибытие или отъезд</span></li>
    <li><strong>Сверка</strong><span>Выбор основной записи</span></li>
    <li><strong>Разъединение</strong><span>Личные данные снова видны</span></li>
  </ol>
</div>

<details class="requirements-drawer" id="technical-appendix">
<summary><span><strong>Техническое приложение для команды разработки</strong><span>Полный каталог полей, правил, API-контрактов, критериев и бэклога. Клиенту не нужно читать его для согласования сценариев.</span></span><b aria-hidden="true">+</b></summary>
...
- **SCOPE-02.** Публичная шапка содержит один продуктовый пункт «Лиды, туры и сводная». Внутри manager-сценария доступны «Туры → Туристы → Финансы → Лиды → Чаты»; для `viewer/escort` сохраняется оболочка первой редакции «Задачи → Туристы» и условные «Финансы», а разрешённые чаты открываются из тура или туриста.
...
- **SCOPE-04.** Прежнее ТЗ приложения гида и рабочие приложения не изменяются; его работа зафиксирована как обязательная регрессионная база, а меняется только изолированный прототип текущей задачи.
- **SCOPE-05.** Markdown-файл является единственным источником требований текущей задачи.

## 1.1. Текущая разработка и основные пользовательские сценарии

Задача текущей разработки состоит в том, чтобы проверить на кликабельном прототипе основную ежедневную работу веб-«Сводной таблицы» до переноса в рабочее приложение и подключения production API.

Прототип можно переносить в рабочее приложение, если менеджер способен подготовить один тур целиком с телефона, а viewer с подписью «Гид» или сопровождающий получает разрешённые данные и отмечает фактическое выполнение операций без доступа к лидам и закрытым персональным полям.
...
| Обсудить работу с сотрудниками | Manager / Admin | «Чаты» → «Команда» → общий, личный, групповой, лид или тур | Сообщение и файл получают только участники служебной комнаты |
| Ответить клиенту внутри UNIQUE | Manager / Admin / назначенный гид | «Чаты → Клиенты» либо контекстная кнопка в туре или туристе | Личный диалог привязан к контакту, общий чат к туру; адресат виден до отправки |
| Продолжить внешний диалог | Manager / Admin | Лид → «Чат» → Wazzup | Открывается внешний канал нужного лида без смешивания с внутренней историей UNIQUE |
...
- **PROJECT-01.** Текущая разработка проверяет основные сценарии веб-«Сводной таблицы» в кликабельном прототипе.
- **PROJECT-02.** В границы разработки входят единая сводная тура, одна карточка туриста, индивидуальная и групповая логистика, документы, операционные статусы, чаты, роли, остатки оплаты и mock-справочник.
- **PROJECT-03.** Рабочее приложение, production API, база данных и серверные права подключаются после проверки и согласования прототипа.
...
## 1.2. Публикация прототипа без слияния
...
- **DELIVERY-01.** Все файлы прототипа находятся в `prototype/mobile-crm-2026-08-03`; `main` и `develop` не меняются.
- **DELIVERY-02.** Для текущего этапа не создаются PR и merge; Pages публикует прототипную ветку.
- **DELIVERY-03.** `https://nvrtsky.github.io/unique-guide-app/tour-operations.html` отвечает HTTP 200 и показывает текущий единый прототип.
...
- **SCHEDULE-01.** Срок разработки составляет 30 календарных дней. Отсчёт начинается после письменного утверждения прототипа и получения необходимых доступов.
- **SCHEDULE-02.** План распределяет работу так: дни 1–5 на уточнение интерфейса и контрактов, дни 6–20 на разработку и интеграцию, дни 21–26 на тесты и исправления, дни 27–30 на выпуск и передачу.
- **SCHEDULE-03.** Период ожидания недостающего решения, материала или доступа со стороны клиента не входит в срок; команда письменно фиксирует дату остановки и возобновления отсчёта.
...
├─ Лиды [только admin / manager]
   └─ Карточка лида
      ...
      ├─ Документы
      └─ Задачи
└─ Чаты [только admin / manager]
   ├─ Команда
   ├─ Клиенты
   └─ Wazzup → карточка доступного лида

Viewer / Escort · первая редакция
├─ Задачи
├─ Туристы
├─ Финансы [только при действующем доступе]
└─ Контекстные входы в личный чат туриста и общий чат тура
```
...
- **IA-01.** Для admin и manager нижнее меню идёт `Туры → Туристы → Финансы → Лиды → Чаты`, если Finance разрешён; для viewer и escort сохраняется `Задачи → Туристы → Финансы при доступе`, без «Лидов» и отдельного центра чатов.
...
«Финансы» в текущей разработке означают **остаток по оплате заявки**, а не бухгалтерию. Одна карточка соответствует одному лиду/заявке и содержит всех его туристов. При отметке оплаты не меняются сумма, валюта, состав туристов и логистика.
...
- **FIN-13.** Прибыль, расходы, акты, отчёты и бухгалтерские проводки не входят в текущую разработку.
...
## 9.4. Чаты сотрудников, клиентов и Wazzup

Рабочая версия использует три независимых канала общения. Сервер проверяет доступ к каждой комнате и каждому лиду; скрытая кнопка сама по себе не считается защитой.

| Контур | Связь и адресат | Вложения | История и права |
|---|---|---|---|
| Команда | `general`, прямой диалог, группа, комната лида или тура | Любой файл до 10 МБ | Участник видит свою комнату, редактирует и удаляет свои сообщения |
| Личный чат клиента | Один диалог на `contactId`; `eventId` задаёт контекст тура, но не создаёт новый диалог | До 10 изображений или PDF, общий объём до 50 МБ | Состав сотрудников определяет сервер по назначению и правам; клиент видит тот же диалог в портале |
| Общий чат тура | Одна комната на `eventId` для участников тура и допущенных сотрудников | До 10 изображений или PDF, общий объём до 50 МБ | Автор меняет своё сообщение; администратор чата может удалить любое; доступ сохраняется до 30 дней после завершения тура |
| Wazzup | Внешний чат WhatsApp, Telegram, Instagram или Max из карточки конкретного лида | По правилам Wazzup | История, доставка и прочтение принадлежат iframe и не копируются во внутренние чаты UNIQUE |

- **CHAT-01.** Manager и admin получают пятый пункт нижнего меню «Чаты» с вкладками «Команда», «Клиенты» и «Wazzup»; счётчик показывает разрешённые непрочитанные внутренние сообщения.
- **CHAT-02.** Для `viewer` и `escort` нижнее меню первой редакции не меняется. Личный чат туриста и общий чат тура открываются из прежних контекстных кнопок при наличии серверного доступа.
- **CHAT-03.** Служебные комнаты поддерживают типы `general | direct | group | lead | event`. Клиент не видит их сообщения и вложения.
- **CHAT-04.** В служебной комнате сотрудник отправляет текст и любой файл размером до 10 МБ, изменяет и удаляет только собственные сообщения.
- **CHAT-05.** Личный клиентский диалог имеет один стабильный поток на `contactId`. Необязательный `eventId` показывает контекст выбранного тура без копирования истории.
- **CHAT-06.** Общий клиентский чат имеет одну комнату на `eventId`. Сервер включает туристов с участием в туре, менеджеров их лидов, администраторов чата, сопровождающего и назначенных гидов.
- **CHAT-07.** Личный и общий клиентские чаты принимают за одну отправку до десяти изображений или PDF общим объёмом до 50 МБ.
- **CHAT-08.** В клиентском чате автор изменяет собственное сообщение; администратор чата может удалить любое сообщение в рамках действующих серверных прав.
- **CHAT-09.** Общий чат тура доступен не дольше 30 дней после завершения тура. Затем сервер закрывает комнату и удаляет её вложения по действующему правилу хранения.
- **CHAT-10.** Перед отправкой экран явно показывает адресата: «только сотрудники», имя клиента либо название и состав тура.
- **CHAT-11.** Для списка и комнаты предусмотрены loading, empty, error/retry, forbidden и offline. В offline загруженная история читается, а отправка, изменение и удаление заблокированы.
- **CHAT-12.** Deep link не расширяет права. Сервер повторно проверяет участника комнаты, доступ к контакту, лиду или туру до выдачи истории и перед каждой записью.
- **CHAT-13.** Непрочитанные сообщения считаются отдельно по внутренним комнатам. Состояние Wazzup не добавляется к этому счётчику, потому что CRM не хранит его историю доставки и прочтения.
- **CHAT-14.** Прототип использует только mock-сообщения и не отправляет данные сотрудникам, клиентам, в портал или внешний сервис.
- **CHAT-15.** Личный клиентский чат доступен администратору чата, сопровождающему и назначенным городским гидам. Менеджер получает резервный доступ, если у тура ещё нет команды; прошлое назначение участника сохраняет доступ к его истории. Роль admin сама по себе не даёт доступ ко всем личным чатам.

- **WAZ-01.** Wazzup открывается из вкладки «Чат» карточки доступного лида и поддерживает WhatsApp, Telegram, Instagram и Max.
- **WAZ-02.** Экран различает «ключ сохранён», «канал активен», «загрузка» и «ошибка». Сохранённый ключ сам по себе не подтверждает доступность канала.
- **WAZ-03.** Перед открытием iframe пользователь видит лид, получателя и выбранный мессенджер. Переход из общего центра приводит в карточку того же лида.
- **WAZ-04.** Внутренние чаты UNIQUE и Wazzup не объединяют историю, непрочитанные сообщения, доставку и прочтение.
- **WAZ-05.** Перед выпуском сервер проверяет право пользователя на указанный лид, iframe принимает сообщения только от разрешённого `origin`, а webhook проверяет подпись провайдера.
- **WAZ-06.** Ошибка загрузки iframe не закрывает лид и не выдаётся за отсутствие сообщений; пользователь получает действие «Повторить».
...
| Центр служебных чатов | да | разрешённые комнаты | нет | нет |
| Личный / общий чат клиента | по серверному доступу | по серверному доступу | назначенный тур | назначенный тур / турист |
| Wazzup из лида | доступный лид | назначенный лид | нет | нет |
...
11. получить доступные служебные и клиентские комнаты, историю и непрочитанные сообщения;
12. отправить, изменить или удалить сообщение по правилам выбранного контура;
13. получить безопасную конфигурацию Wazzup для доступного лида без выдачи секретного ключа клиенту.
...
- **API-12.** Сервер проверяет членство в служебной комнате, доступ к `contactId/eventId` и право на лид при чтении и каждом изменении чата.
- **API-13.** Мобильный клиент использует существующие серверные модели личного и общего клиентского чата, сроки хранения и правила вложений; второй поток сообщений не создаётся.
- **API-14.** Wazzup iframe получает только данные доступного лида. Клиент проверяет `event.origin`, сервер проверяет право на лид, webhook принимает событие только с действительной подписью.

### 13.1. Проверенный web-контракт

| Источник | Что является каноном для мобильной версии |
|---|---|
...
| `Chat.tsx`, `shared/schema.ts` | Служебные комнаты `general/direct/group/lead/event`, участники, непрочитанные сообщения и права автора |
| Клиентские chat routes и portal chat | Один личный поток на контакт, одна общая комната на тур, вложения, роли и срок доступа |
| `Wazzup24Chat.tsx` | Внешний iframe из карточки лида, состояние настройки и сохранение channel ID |
...
Подробная декомпозиция опубликована на странице <a class="inline-action" href="commercial-proposal.html">Открыть подробное КП</a>. Фиксированная стоимость текущей разработки составляет **390 000 ₽**.

Актуальная декомпозиция задач и распределение стоимости находятся в КП. Функции первой редакции, включая финансы, сохраняются в продукте, но повторно в текущем КП не оплачиваются.

Уже работающая редакция для гида полностью сохраняется как обязательное условие совместимости. Она не оценивается отдельной строкой и не входит в расчёт. Публичные ставки, зарплатные обзоры, проекты с нуля и лицензии CRM используются только как внешний ориентир.

- **CP-01.** Фиксированная стоимость текущей разработки равна 390 000 ₽ и соответствует актуальной декомпозиции на странице КП.
- **CP-02.** Работающая редакция для гида сохраняется без отдельной цены в текущем расчёте.
- **CP-03.** Суммы актуальных блоков и задач независимо сходятся к 390 000 ₽; уже оплаченные функции первой редакции повторно не включаются.
...
- **AC-01.** Публичная витрина содержит один пункт «Лиды, туры и сводная». Нижняя навигация manager/admin идёт `Туры → Туристы → Финансы → Лиды → Чаты`; у `viewer/escort` сохраняется `Задачи → Туристы → Финансы при доступе`, без «Лидов» и отдельного центра чатов.
...
- **AC-11.** Вкладка лида «Чат» показывает mock Wazzup без внешнего запроса и различает сохранённый ключ, активный канал, загрузку и ошибку.
...
- **AC-60.** `commercial-proposal.html` показывает фиксированную стоимость 390 000 ₽, не содержит прежних сумм, коэффициента или отдельного резерва и отделяет проектную оценку от рыночных ориентиров.
...
- **AC-62.** ТЗ и опубликованный прототип не используют обозначение удалённого этапа; текущий результат называется прототипом или текущей разработкой.
- **AC-63.** В паспорте указан срок 30 календарных дней после письменного утверждения прототипа и получения доступов; график показывает периоды 1–5, 6–20, 21–26 и 27–30 дней.
- **AC-64.** Manager/admin открывает пятый пункт «Чаты» и переключает «Команда / Клиенты / Wazzup». Viewer/escort сохраняет прежнее нижнее меню и открывает разрешённый личный или общий чат из контекста.
- **AC-65.** Служебный чат явно подписан «только сотрудники», поддерживает общий, прямой, групповой, lead- и event-контекст и в прототипе имитирует отправку текста и файла без сетевого запроса.
- **AC-66.** Личный клиентский чат показывает конкретного получателя и один поток на контакт; общий чат показывает конкретный тур. Оба контура не смешиваются со служебной перепиской.
- **AC-67.** Wazzup открывается из доступного лида, показывает мессенджер и получателя, не добавляет внешние delivery/read в счётчик внутренних чатов и сохраняет контекст лида при ошибке.
- **AC-68.** Offline, loading, empty, error/retry и forbidden проверяются отдельно для списка чатов и комнаты; ни одно заблокированное действие не показывает ложный успех.
- **AC-69.** Проверка рабочей версии подтверждает серверные права на комнату, контакт, тур и лид, лимиты вложений и закрытие общего чата тура через 30 дней после завершения.
- **AC-70.** Перед production-выпуском Wazzup проверяет `origin`, авторизацию пользователя на лид и подпись webhook; отсутствие любой проверки блокирует выпуск.
...
- **NG-11.** Не превращать Wazzup24 iframe в новый нативный CRM-мессенджер в текущей разработке.
...
- **NG-14.** Не создавать PR и не сливать прототип в `main` или `develop`. Публикация GitHub Pages выполняется только из прототипной ветки и не разрешает merge.
...
- **BL-05.** Нативная замена Wazzup iframe с хранением внешней истории, доставки и прочтения внутри CRM.
...
3. Проверить состояния Wazzup «ключ сохранён / канал активен / загрузка / ошибка», два DOCX-документа и полный цикл задачи `todo → in_progress → done → todo`.
...
23. Открыть `commercial-proposal.html`, проверить итог 390 000 ₽, актуальную декомпозицию, отсутствие повторной оплаты функций первой редакции и прямые ссылки на источники.
...
28. Для manager/admin открыть «Чаты» и проверить три вкладки, служебную комнату, личный клиентский диалог, общий чат тура, непрочитанные сообщения, поиск и состояния loading/empty/error/forbidden/offline.
29. Открыть общий чат тура из роли viewer и личный чат из карточки разрешённого туриста. Убедиться, что нижнее меню первой редакции не изменилось, а адресат указан до отправки.
30. Проверить mock-отправку текста и вложений без сетевых запросов. Для будущей рабочей версии отдельно проверить лимит 10 МБ в служебном чате, до 10 изображений/PDF общим объёмом до 50 МБ в клиентском чате и серверный отказ для постороннего пользователя.
31. Проверить Wazzup из назначенного лида для WhatsApp, Telegram, Instagram и Max. История и delivery/read не должны появляться во внутренних чатах; production-выпуск блокируется без проверки `origin`, доступа к лиду и подписи webhook.
32. Сверить паспорт и график: срок составляет 30 календарных дней после письменного утверждения прототипа и получения доступов, этапы занимают дни 1–5, 6–20, 21–26 и 27–30.
